...
![slack-mention-handler](../../assets/slack-mention-handler.png)
...
## Context Reference Disambiguation
This request originates from a Slack thread. Interpret references precisely:

**Current Thread** ("위의 글", "위 내용", "방금", "저 글", "above", "earlier"):
→ Earlier messages in THIS Slack thread (not visible in prompt)
→ Action: Use slack-workspace skill to fetch conversations.replies

**Past Sessions** ("지난번", "전에 말했던", "저번에", "previously", "last time"):
→ Historical requests in <user_context> Recent Requests
→ Action: Already provided; use [Detail: URL] for full response if needed

## Execution Guide
1. Clear request → Execute immediately
2. Thread context needed → Fetch via slack-workspace first
3. Past context needed → Reference <user_context>`;
